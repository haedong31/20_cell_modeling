...
function [RATES, ALGEBRAIC] = computeRates(X, t, STATES, holding_p, holding_t, P1, P1_t, Ek)
    ...
    % Ito
    % A71; alpha_a
    ALGEBRAIC(:,1) =  X(1).*exp( 0.0357700.*(ALGEBRAIC(:,16)+30.0));
    % A72; beta_a
    ALGEBRAIC(:,2) =  0.395600.*exp(  - 0.0623700.*(ALGEBRAIC(:,16)+30.0));
    % A73; alpha_i
    ALGEBRAIC(:,3) = ( 0.000152000.*exp( - (ALGEBRAIC(:,16)+13.5)./7.0))./( 0.00670830.*exp( - (ALGEBRAIC(:,16)+33.5)./7.0)+1.00000);
    % A74; beta_i
    ALGEBRAIC(:,4) = ( X(2).*exp((ALGEBRAIC(:,16)+33.5)./7.0))./( 0.0513350.*exp((ALGEBRAIC(:,16)+33.5)./7.0)+1.00000);
    % A69; ato_f
    RATES(:,1) =  ALGEBRAIC(:,1).*(1.00000 - STATES(:,1)) -  ALGEBRAIC(:,2).*STATES(:,1);
    % A70; ito_f
    RATES(:,2) =  ALGEBRAIC(:,3).*(1.00000 - STATES(:,2)) -  ALGEBRAIC(:,4).*STATES(:,2);
    % A67; I_Kto,f
    ALGEBRAIC(:,5) =  X(7).*power(STATES(:,1), 3.00000).*STATES(:,2).*(ALGEBRAIC(:,16) - Ek);

    % IKslow1
    % A78; a_ss
    ALGEBRAIC(:,6) = 1.00000./(1.00000+exp( - (ALGEBRAIC(:,16)+X(3))./7.7));
    % A79; i_ss
    ALGEBRAIC(:,7) = 1.00000./(1.00000+exp((ALGEBRAIC(:,16)+45.2)./5.7));
    % A90; tau_aur
    ALGEBRAIC(:,8) =  0.493000.*exp(  - 0.0629000.*ALGEBRAIC(:,16)) + 2.058;
    % A91; tau_iur
    ALGEBRAIC(:,9) = X(4) - 170.000./(1.00000+exp((ALGEBRAIC(:,16)+45.2)./5.7));
    % A88; aur
    RATES(:,3) = (ALGEBRAIC(:,6) - STATES(:,3))./ALGEBRAIC(:,8);
    % A89; iur
    RATES(:,4) = (ALGEBRAIC(:,7) - STATES(:,4))./ALGEBRAIC(:,9);
    % A87; I_Kur
    ALGEBRAIC(:,10) =  X(8).*STATES(:,3).*STATES(:,4).*(ALGEBRAIC(:,16) - Ek);
    
    % IKslow2
    % A78; a_ss
    ALGEBRAIC(:,11) = 1.00000./(1.00000+exp( - (ALGEBRAIC(:,16)+X(5))./7.7));
    % A79; i_ss
    ALGEBRAIC(:,12) = 1.00000./(1.00000+exp((ALGEBRAIC(:,16)+45.2)./5.7));
    % A90; tau_aur
    ALGEBRAIC(:,13) =  0.493000.*exp(  - 0.0629000.*ALGEBRAIC(:,16)) + 2.058;
    % A91; tau_iur
    ALGEBRAIC(:,14) = X(6) - 170.000./(1.00000+exp((ALGEBRAIC(:,16)+45.2)./5.7));
    % A88; aur
    RATES(:,5) = (ALGEBRAIC(:,11) - STATES(:,5))./ALGEBRAIC(:,13);
    % A89; iur
    RATES(:,6) = (ALGEBRAIC(:,12) - STATES(:,6))./ALGEBRAIC(:,14);
    % A87; I_kUR
    ALGEBRAIC(:,15) =  X(9).*STATES(:,5).*STATES(:,6).*(ALGEBRAIC(:,16) - Ek);
