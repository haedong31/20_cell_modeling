...
function [RATES, ALGEBRAIC] = computeRates(X, t, STATES, holding_p, holding_t, P1, P1_t, Ek)
    ...
    % A90; tau_aur
    ALGEBRAIC(:,3) =  0.493000.*exp(  - 0.0629000.*ALGEBRAIC(:,6)) + X(5);
    % A91; tau_iur
    ALGEBRAIC(:,4) = X(6) - X(7)./(1.00000+exp((ALGEBRAIC(:,6)+45.2)./5.7));
    % A88; aur
    RATES(:,1) = (ALGEBRAIC(:,1) - STATES(:,1))./ALGEBRAIC(:,3);
    % A89; iur
    RATES(:,2) = (ALGEBRAIC(:,2) - STATES(:,2))./ALGEBRAIC(:,4);
    % A87; I_kUR
    ALGEBRAIC(:,5) =  X(8).*STATES(:,1).*STATES(:,2).*(ALGEBRAIC(:,6) - Ek);
