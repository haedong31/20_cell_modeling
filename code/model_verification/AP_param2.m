...
function [VOI, STATES, ALGEBRAIC, CONSTANTS] = AP_param(X, sim_time)
    % This is the "main function".  In Matlab, things work best if you rename this function to match the filename.
   [VOI, STATES, ALGEBRAIC, CONSTANTS] = solveModel(X, sim_time);
end
...
function [VOI, STATES, ALGEBRAIC, CONSTANTS] = solveModel(X, sim_time)
    ...
    [VOI, STATES] = ode15s(@(VOI, STATES)computeRates(X, VOI, STATES, CONSTANTS), tspan, INIT_STATES, options);

    % Compute algebraic variables
    [RATES, ALGEBRAIC] = computeRates(X, VOI, STATES, CONSTANTS);
    ALGEBRAIC = computeAlgebraic(X, ALGEBRAIC, CONSTANTS, STATES, VOI);
end
...
function [STATES, CONSTANTS] = initConsts()
    ...
    STATES(:,42) = 0.417069e-3;
    STATES(:,43) = 0.998543;

    STATES(:,44) = 0.417069e-3;
    STATES(:,45) = 0.998543;
...
function [RATES, ALGEBRAIC] = computeRates(X, VOI, STATES, CONSTANTS)
    ...
    ALGEBRAIC(:,60) =  -91.1;
    ALGEBRAIC(:,61) =  X(6).*power(STATES(:,29), 3.00000).*STATES(:,30).*(STATES(:,1) - ALGEBRAIC(:,60));
    ...
    % IKslow1
    ALGEBRAIC(:,72) = 1.00000./(1.00000+exp( - (STATES(:,1)+X(7))./X(8)));
    ALGEBRAIC(:,73) = 1.00000./(1.00000+exp((STATES(:,1)+X(9))./X(10)));
    ALGEBRAIC(:,74) =  0.493000.*exp(  - 0.0629000.*STATES(:,1))+2.05800;
    ALGEBRAIC(:,75) = X(11) - 170.000./(1.00000+exp((STATES(:,1)+45.2000)./5.70000));
    ALGEBRAIC(:,76) =  X(12).*STATES(:,42).*STATES(:,43).*(STATES(:,1) - ALGEBRAIC(:,60));
    RATES(:,42) = (ALGEBRAIC(:,72) - STATES(:,42))./ALGEBRAIC(:,74);
    RATES(:,43) = (ALGEBRAIC(:,73) - STATES(:,43))./ALGEBRAIC(:,75);

    % IKslow2
    ALGEBRAIC(:,77) = 1.00000./(1.00000+exp( - (STATES(:,1)+X(13))./X(14)));
    ALGEBRAIC(:,78) = 1.00000./(1.00000+exp((STATES(:,1)+X(15))./X(16)));
    ALGEBRAIC(:,79) =  0.493000.*exp(  - 0.0629000.*STATES(:,1))+2.05800;
    ALGEBRAIC(:,80) = X(17) - 170.000./(1.00000+exp((STATES(:,1)+45.2000)./5.70000));
    ALGEBRAIC(:,81) =  X(18).*STATES(:,44).*STATES(:,45).*(STATES(:,1) - ALGEBRAIC(:,60));
    RATES(:,44) = (ALGEBRAIC(:,77) - STATES(:,44))./ALGEBRAIC(:,79);
    RATES(:,45) = (ALGEBRAIC(:,78) - STATES(:,45))./ALGEBRAIC(:,80);

    RATES(:,1) =  - (ALGEBRAIC(:,47)+ALGEBRAIC(:,49)+ALGEBRAIC(:,51)+ALGEBRAIC(:,55)+ALGEBRAIC(:,58)+ALGEBRAIC(:,59)+ALGEBRAIC(:,69)+ALGEBRAIC(:,61)+ALGEBRAIC(:,66)+ALGEBRAIC(:,71)+ALGEBRAIC(:,1)+ALGEBRAIC(:,76)+ALGEBRAIC(:,81));
    RATES = RATES';
end

% Calculate algebraic variables
function ALGEBRAIC = computeAlgebraic(X, ALGEBRAIC, CONSTANTS, STATES, VOI)
    ALGEBRAIC(:,2) = 1.00000 - (STATES(:,11)+STATES(:,9)+STATES(:,10));
    ALGEBRAIC(:,5) =  0.180640.*exp( 0.0357700.*(STATES(:,1)+X(1)));
    ALGEBRAIC(:,15) =  0.395600.*exp(  - 0.0623700.*(STATES(:,1)+X(2)));
    ALGEBRAIC(:,6) = ( 0.000152000.*exp( - (STATES(:,1)+X(3))./7.00000))./( 0.00670830.*exp( - (STATES(:,1)+33.5000)./7.00000)+1.00000);
    ALGEBRAIC(:,16) = ( 0.000950000.*exp((STATES(:,1)+X(4))./X(5)))./( 0.0513350.*exp((STATES(:,1)+X(4))./X(5))+1.00000);
    ALGEBRAIC(:,7) = 1.00000./(1.00000+exp( - (STATES(:,1)+22.5000)./7.70000));
    ALGEBRAIC(:,17) =  0.493000.*exp(  - 0.0629000.*STATES(:,1))+2.05800;
    ALGEBRAIC(:,8) = 1.00000./(1.00000+exp((STATES(:,1)+45.2000)./5.70000));
    ALGEBRAIC(:,18) = 270.000+1050.00./(1.00000+exp((STATES(:,1)+45.2000)./5.70000));
